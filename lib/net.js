...
Net.prototype.listen = function listen(callback) {
  var listeningListener = this.listeningListener
...
function(err) {
    if( 'EADDRINUSE' === err.code && attempts < max_attempts ) {
      attempts++;
      setTimeout( bind_socket, 100 + Math.floor(Math.random()*222) )
      return;
    }
...
*/
  ...
  var port, udpSocket = this.udpSocket, 
      udp = this.udp;
  //, attempts = 0, max_attempts = 11;

  function bind_socket() {
    /*


    if( _.isFunction( udp.port ) ) {
      port = udp.port() : 
    }
     */
